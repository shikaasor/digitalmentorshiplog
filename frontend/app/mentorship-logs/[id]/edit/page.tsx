...
export default function EditMentorshipLogPage() {
  ...
  const onSubmit = async (data: MentorshipLogUpdate) => {
    try {
      setLoading(true)
      setError(null)

      // Update the mentorship log
      await mentorshipLogsService.update(logId, data)

      // Upload files to the server if any
      if (uploadedFiles.length > 0) {
        try {
          await attachmentsService.upload(logId, uploadedFiles)
        } catch (uploadErr: any) {
          // Log updated successfully but file upload failed
          console.error('File upload failed:', uploadErr)
          setError(`Log updated but file upload failed: ${uploadErr.message}`)
          // Still navigate to the log page so user can retry upload later
        }
      }

      router.push(`/mentorship-logs/${logId}`)
      router.refresh()
    } catch (err: any) {
      setError(err.message || 'Failed to update mentorship log')
    } finally {
      setLoading(false)
    }
  }
  ...
  const saveDraft = async () => {
    const data = watch()
    await onSubmit(data as MentorshipLogUpdate)
  }
  ...
  const submitForApproval = async () => {
    try {
      ...
      // First update the log with current form data
      const data = watch()
      await mentorshipLogsService.update(logId, data as MentorshipLogUpdate)
      ...
      // Then submit for approval (change status to submitted)
      await mentorshipLogsService.submit(logId)

      router.push(`/mentorship-logs/${logId}`)
      router.refresh()
    } catch (err: any) {
      setError(err.message || 'Failed to submit mentorship log')
    } finally {
      setLoading(false)
    }
  }
                ...
                <div className="space-y-2">
                  {existingAttachments.map((attachment) => (
                    ...
                    >
                      ...
                      <a
                        href={`${process.env.NEXT_PUBLIC_API_URL}${attachment.file_path}`}
                        target="_blank"
